...
class ImageLibrary
{
    ...
    public $water;
    public $filename;
    public $font_file = __DIR__ . '/fonts/GraphikLCG-Bold.ttf';
    
    public function ftext($text){

        return imagefttext($this->nativeimg, 42, 0, 96, 1200, 0, $this->font_file, $text);
    }

    public function validatortext($text){
        if(iconv_strlen($text) > 70)
            return false;
        else {
            $validationtext = wordwrap($text, 35, "\n");
    ...
    public function watermark($file, $source=false)
    {
        ...
        $dest_x = ($water_width - 50);
        $dest_y = ($img_height - $water_height - 90);
        
        $cut = imagecreatetruecolor($water_width, $water_height);

        //-----------------------
        $bg_color = imageColorAllocate($this->water, 0, 0, 0); //Делаем фон прозрачным
        imagecolortransparent ($this->water, $bg_color);

        for ($x = 0; $x < $water_width; $x++) {
            for ($y = 0; $y < $water_height; $y++) {
                $color = imagecolorat($this->water, $x, $y);
                if ($color == 0) {
                    imageSetPixel($this->water, $x, $y, $bg_color);
                }
            }
        }
        //----------------------
