...
class AppBasket extends CBitrixComponent {
    ...
    public function onPrepareComponentParams($params) {
        ...
        if($this->initComponentTemplate()) {
            ...
            if($template instanceof CBitrixComponentTemplate && $template->GetSiteTemplate() == '' && $template->GetName() === '.default') {
    ...
    protected function basketOrderAction() {
        ...
        if(empty($basketRefresh) && !empty($basketOrder) && empty($result['WARNING_MESSAGE'])) {
            if(!array_key_exists('paypalbutton_x', $postList) && !array_key_exists('paypalbutton_y', $postList)) {
                if(!empty($postList['CASH']))
                    $_SESSION['SALE_BASKET_CASH'] = $postList['CASH'];
    ...
    public function getBasketItems() {
        ...
        foreach($result["GRID"]["ROWS"] as &$arItem) {
            if(!empty($arItem["PROPERTY_OLD_PRICE_VALUE"]) && $arItem["DISCOUNT_PRICE_PERCENT"] == 0) {
                ...
                $arItem["DISCOUNT_PRICE"] = Price::roundPrice($arItem["~PRICE_TYPE_ID"], $arItem["FULL_PRICE"] - $arItem["PRICE"], $arItem["CURRENCY"]);
                $arItem["DISCOUNT_PRICE_PERCENT"] = Sale\Discount::calculateDiscountPercent($arItem["FULL_PRICE"], $arItem["DISCOUNT_PRICE"]);
                $arItem["DISCOUNT_PRICE_PERCENT_FORMATED"] = Sale\BasketItem::formatQuantity($arItem["DISCOUNT_PRICE_PERCENT"])."%";
                $basketBasePrice += Price::roundPrice($arItem["~PRICE_TYPE_ID"], $arItem["DISCOUNT_PRICE"] * $arItem["QUANTITY"], $arItem["CURRENCY"]);
                unset($oldPrice);
            }
        }
        unset($arItem);
        ...
        if($hasOldPrice) {
            $result["PRICE_WITHOUT_DISCOUNT"] = CCurrencyLang::CurrencyFormat($basketBasePrice, $result["CURRENCY"], true);
            $result["DISCOUNT_PRICE_ALL"] = PriceMaths::roundPrecision($basketBasePrice - $result["allSum"]);
            $result["DISCOUNT_PRICE_FORMATED"] = $result["DISCOUNT_PRICE_ALL_FORMATED"] = CCurrencyLang::CurrencyFormat($result["DISCOUNT_PRICE_ALL"], $result["CURRENCY"], true);
        }
        ...
        foreach($result["GRID"]["ROWS"] as &$arItem) {
            if(!empty($arItem["PROPERTY_M2_COUNT_VALUE"])) {
                ...
                if($arItem["MEASURE_SYMBOL_INTL"] == "pc. 1") {
                    ...
                    $SqMMeasureRatio = round($arItem["MEASURE_RATIO"] / $sqMCount, 2);

                    $arItem["PC_QUANTITY"] = $arItem["QUANTITY"];
                    $arItem["SQ_M_QUANTITY"] = round(($arItem["QUANTITY"] * $SqMMeasureRatio) / $arItem["MEASURE_RATIO"], 2);
        ...
        if(!empty($objectsIds)) {
            ...
            if(!empty($arObjects)) {
                foreach($result["GRID"]["ROWS"] as &$arItem) {
                    if(!empty($arItem["PROPERTY_OBJECT_VALUE"]) && array_key_exists($arItem["PROPERTY_OBJECT_VALUE"], $arObjects))
                        $arItem["PROPERTY_OBJECT_FULL_VALUE"] = $arObjects[$arItem["PROPERTY_OBJECT_VALUE"]];
                }
                unset($arItem);
            }
            unset($arObjects);
