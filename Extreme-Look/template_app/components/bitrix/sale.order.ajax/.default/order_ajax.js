...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		createPaySystemItem: function(item)
		{
			...
			/* KN */
			if(this.result.IS_PARTNER == "Y" && window.IS_PARTNER == undefined){
				window.IS_PARTNER = 'Y';
			}
			// if(window.IS_PARTNER == "Y" && paySystemId == 11){
			// 	itemNode = BX.create('DIV');
			// } 
			if(this.result.IS_PARTNER == "N" && paySystemId == 8){
				itemNode = BX.create('DIV');
			}
		...
		editFadePaySystemContent: function(node)
		{
			...
			if (selectedPaySystem && selectedPaySystem.PSA_NAME)
			{
				...
				if(selectedPaySystem.DESCRIPTION.length > 0){
					addedHtml += " - " + selectedPaySystem.DESCRIPTION + '<br />'; /* KN */
				}
				addedHtml += '</div>';
			}
		...
		validateLocation: function(input, arProperty, fieldName)
		{
			...
			/* KN * forbidden cities >> */
			var forbidden_cities = [];
			$('#non_selected_cities div').each(function () {
				forbidden_cities.push($(this).text().toLowerCase());
			});

			var if_chel = $('.bx-ui-sls-fake').val().toLowerCase().indexOf('челябинск');
			if(if_chel >=  0){ /* SHOW / HIDE SAMOVYVOZ */

			}
			$('.bx-ui-sls-fake').each(function () {
				if($.inArray($(this).val().toLowerCase(), forbidden_cities) >= 0){
					errors.push('В выбранном городе ('+ $(this).val()  +') заказ не доступен. Вы можете заказать товар через официального представителя');
				}
			});
			/* << KN forbidden cities */
			return errors;
