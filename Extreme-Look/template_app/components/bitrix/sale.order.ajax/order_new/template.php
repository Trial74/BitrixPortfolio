...
if(getPartner() || getNewPartner())addServiceByPartner();//Мой код Добавляем сервисный сбор если партнёр
...
$specSity = bxSpecSity();//Мой код Список городов эксклюзивов
?>
<script>
    var specSity = <?=json_encode($specSity)?>,
        uspartner = <?=json_encode(getPartner() || getNewPartner())?>,
        uscash = <?=json_encode(getUserCashBack())?>,
        totalordersumm = <?=json_encode($_SESSION["MY_PARAMS"][1])?>,
        administrator = <?=json_encode($_SESSION["MY_PARAMS"][2])?>,
        enroll = <?=json_encode($_GET["cash"] == 'Y' ? $_GET["cash"] : false)?>;
</script>
<?
...
{
    case 'enext':
        $ord_template = 'Сайт'; break;
    case 'mobileapp':
        $ord_template = 'Приложение'; break;
}
...
?>
<NOSCRIPT>
	<div style="color:red">1<?=Loc::getMessage("SOA_NO_JS")?></div>
</NOSCRIPT>
<?
...
<form action="/?extreme-mobile=Y&page=personal/order" method="POST" name="ORDER_FORM" id="bx-soa-order-form" enctype="multipart/form-data">
    <?=bitrix_sessid_post()?>
    <?if(strlen($arResult["PREPAY_ADIT_FIELDS"]) > 0)
	    echo $arResult["PREPAY_ADIT_FIELDS"];?>
    <input type="hidden" name="action" value="saveOrderAjax">
    <input type="hidden" name="location_type" value="code">
    <input type="hidden" name="ORDER_PROP_39" value="<?=$ord_template?>">
    <input type="hidden" name="ORDER_PROP_46" value="<?=getAgreement()?>">
    <input type="hidden" name="ORDER_PROP_45" value="">
    ...
    <!--	MAIN BLOCK	-->
        <div class="col-sm-9 bx-soa">
            ...
            <?if(!isset($arParams['BASKET_POSITION']) || $arParams['BASKET_POSITION'] == 'before'){?>
                ...
                <div id="bx-soa-basket" data-visited="false" class="bx-soa-section bx-active">
                    <div class="bx-soa-section-title-container">
                        ...
                        <div class="col-sm-3 text-right"><a href="javascript:void(0)" class="bx-soa-editstep"><i class="arrow-down-soa"></i></a></div>
                    </div>
                    <div class="bx-soa-section-content container-fluid"></div>
                </div>
            <?}?>
            ...
            <div id="bx-soa-region" data-visited="false" class="bx-soa-section bx-active">
                <div class="bx-soa-section-title-container">
                    <h2 class="bx-soa-section-title col-sm-9"><?=$arParams['MESS_REGION_BLOCK_NAME']?></h2>
                    <div class="col-sm-3 text-right"><a href="" class="bx-soa-editstep"><i class="arrow-down-soa"></i></a></div>
                </div>
                <div class="location_error_message_box"></div>
                <div class="bx-soa-section-content container-fluid"></div>
            ...
            <?if($arParams["DELIVERY_TO_PAYSYSTEM"] == "p2d"){?>
                ...
                <div id="bx-soa-paysystem" data-visited="false" class="bx-soa-section bx-active">
                    <div class="bx-soa-section-title-container">
                        <h2 class="bx-soa-section-title col-sm-9"><?=$arParams['MESS_PAYMENT_BLOCK_NAME']?></h2>
                        <div class="col-sm-3 text-right"><a href="" class="bx-soa-editstep"><i class="arrow-down-soa"></i></a></div>
                    </div>
                    <div class="bx-soa-section-content container-fluid"></div>
                </div>
                ...
                <div id="bx-soa-delivery" data-visited="false" class="bx-soa-section bx-active" <?=($hideDelivery?'style="display:none"':'')?>>
                    <div class="bx-soa-section-title-container">
                        <h2 class="bx-soa-section-title col-sm-9"><?=$arParams['MESS_DELIVERY_BLOCK_NAME']?></h2>
                        <div class="col-sm-3 text-right"><a href="" class="bx-soa-editstep"><i class="arrow-down-soa"></i></a></div>
                    </div>
                    <div class="bx-soa-section-content container-fluid"></div>
                </div>
            ...
            <div id="bx-soa-properties" data-visited="false" class="bx-soa-section bx-active">
                <div class="bx-soa-section-title-container">
                    <h2 class="bx-soa-section-title col-sm-9"><?=$arParams['MESS_BUYER_BLOCK_NAME']?></h2>
                    <div class="col-sm-3 text-right"><a href="" class="bx-soa-editstep"><i class="arrow-down-soa"></i></a></div>
                </div>
                <div class="bx-soa-section-content container-fluid"></div>
            </div>
        ...
        <!--       USER CONSENT     -->
        <label id="bx-soa-consent" class="col-sm-3 bx-soa-consent">
            <input type="checkbox" value="Y" checked>
            <span><?=Loc::getMessage('SOA_OTHER_CONSENT')?></span>
        </label>
        <!--	BUTTON SAVE BLOCK	-->
        <div id="bx-soa-btn-save" class="col-sm-3 bx-soa-sidebar"></div>
    </div>
</form>
...
<script type="text/javascript">
	...
	BX.Sale.OrderAjaxComponent.init({
        ...
        emailsPartners: <?=CUtil::PhpToJSObject(emailPartSaleOrder())?>,
