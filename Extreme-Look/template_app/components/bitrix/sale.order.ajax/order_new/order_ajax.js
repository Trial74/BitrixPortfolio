...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		init: function(parameters)
		{
			...
			this.emailsPartners = parameters.emailsPartners || []
			...
			this.finalBtnSaveBlock = BX('bx-soa-btn-save');
			...
			this.FORBIDDEN_COUNTRIES = parameters.result.FORBIDDEN_COUNTRIES || []; //KN
			this.USER = parameters.result.USER || []; //KN
		...
		{
			var loaderTimer, form, _form, _this = this;
			...
			if (action == 'saveOrderAjax')
			{
				...
				if (form){
					...
					var formData = app.form.convertToData( _form );

					if( formData.DELIVERY_ID[0] != undefined )
						formData.DELIVERY_ID = formData.DELIVERY_ID[0];

					if( formData.PAY_SYSTEM_ID[0] != undefined )
						formData.PAY_SYSTEM_ID = formData.PAY_SYSTEM_ID[0];
					...
					app.request({
						...
						success: function(data, status, xhr) {
							document.location.href = '/?extreme-mobile=Y&page=personal/order&ORDER_ID=' + data.order.ID;
						},
						...
						complete: function(xhr, status) {
							app.preloader.hide();
							BX.proxy(_this.saveOrder, _this);
						}
					});
		...
		animateScrollTo: function(node, duration, shiftToTop)
		{
			...
			$$('.page-content').scrollTop(ghostTop + 30, (duration || 800));
		...
		checkNotifications: function()
		{
			...
			if (!informer) //Мой код вырубил эту херню
			{
		...
		initOptions: function()
		{
			...
			if (this.result.TOTAL)
			{
				...
				this.options.showBonus = enroll === 'Y' ? true : false
			}
		},
		...
		editBasketItemsHeader: function(basketItemsNode)
		{
			...
			for (i = 0; i < this.result.GRID.HEADERS.length; i++)
			{
				...
				headers.push(
					BX.create('DIV', {
						props: {className: 'bx-soa-item-td bx-soa-item-properties'},
						style: {paddingBottom: '5px'},
						children: [
							BX.create('DIV', {
								props: {className: 'bx-soa-item-td-title'},
		...
		createBasketItem: function(basketItemsNode, item, index, active)
		{
			...
			for (i = 0; i < this.result.GRID.HEADERS.length; i++)
			{
				...
				if (currentColumn.id == 'PREVIEW_PICTURE' || currentColumn.id == 'PROPS' || currentColumn.id == 'NOTES')
					continue;
			...
			cols = [
				...
				BX.create('DIV',{
					props: {className: 'bx-soa-container-properties'},
			...
			basketItemsNode.appendChild(
				BX.create('DIV', {
					props: {className: 'bx-soa-item-tr bx-soa-basket-info'},
					children: cols})
			);
		...
		createBasketItemColumn: function(column, allData, active)
		{
			...
			return BX.create('DIV', {
				props: {className: 'bx-soa-item-td bx-soa-item-properties' + (name ? ' soa-item-name' : '')},
				children: [
					textNode
				]
			});
		...
		editPaySystemItems: function(paySystemNode)
		{
			...
			var paySystemItemsContainer = BX.create('DIV', {props: {className: 'col-sm-7 col-xs-12 bx-soa-pp-item-container'}}),
				paySystemItemNode, i;
		...
		editFadePaySystemContent: function(node)
		{
			...
			if (this.isSelectedInnerPayment())
			{
				...
				addedHtml += '<img src="' + imgSrc + '">';
				addedHtml += '<strong>' + this.result.INNER_PAY_SYSTEM.NAME + '</strong>';
				addedHtml += '</div>';
			}
			...
			if (selectedPaySystem && selectedPaySystem.PSA_NAME)
			{
				...
				if(selectedPaySystem.DESCRIPTION.length > 0){
					addedHtml += "<div>" + selectedPaySystem.DESCRIPTION + '</div>';
				}
				addedHtml += '</div>';
			}
		...
		editFadeDeliveryContent: function(node)
		{
			...
			if (selectedDelivery && selectedDelivery.NAME)
			{
				...
				arNodes = [
					BX.create('IMG', {props: {src: imgSrc, alt: ''}}),
				...
				if (this.params.DELIVERY_FADE_EXTRA_SERVICES == 'Y' && BX.util.object_keys(selectedDelivery.EXTRA_SERVICES).length)
				{
					...
					for (i in selectedDelivery.EXTRA_SERVICES)
					{
						if (selectedDelivery.EXTRA_SERVICES.hasOwnProperty(i))
						{
							...
							if (extraService.value && extraService.value != 'N')
							{
								arNodes.push(BX.create('BR'));
				...
				node.appendChild(
					BX.create('DIV', {
						...
						children: 'Рассчитывается менеджером'/*this.getDeliveryPriceNodes(selectedDelivery)*/
					})
				);
			}
			else
		...
		insertStringProperty: function(property, propsItemNode, disabled)
		{
			...
			{	/* .KN.  формирование строкового поля */
				propContainer = BX.create('DIV', {props: {className: 'soa-property-container'}});
				property.appendTo(propContainer);
				propsItemNode.appendChild(propContainer);
				this.alterProperty(property.getSettings(), propContainer);
		...
		validateString: function(input, arProperty, fieldName)
		{
			...
			else
			{
				...
				if (value.length > 0 && arProperty.IS_EMAIL == 'Y')
				{
					...
					if(!this.result.IS_AUTHORIZED && this.emailsPartners.indexOf(value) >= 0)
						errors.push(BX.message('SOA_INVALID_EMAIL_PARTNER'));
				}
			}
		...
		validateLocation: function(input, arProperty, fieldName) {
			...
			if (arProperty.REQUIRED == 'Y' && (value.length == 0 || value == BX.message('SOA_NOT_SPECIFIED')))
				errors.push(field + ' ' + BX.message('SOA_REQUIRED'));
			var selectSity = value.split(','); //Мой код добавляем города эксклюзивы. Разбиваем строку на страна город область
			if (specSity.indexOf(selectSity[0]) != -1) //Ищем в массиве городов эксклюзивов выбранный пользователем город если индекс == 1 то выводим ошибку
				errors.push('В выбранном городе (' + selectSity[0] + ') заказ не доступен. Вы можете заказать товар через официального представителя');
			var partners_groups = [11, 12, 13, 9];
			var user_is_parther = false;
			if (this.result.IS_AUTHORIZED){
				this.USER.GROUPS.forEach(function (group) {
					if (partners_groups.indexOf(parseInt(group)) >= 0) {
						user_is_parther = true;
					}
				});
				var user = this.USER;
				this.FORBIDDEN_COUNTRIES.forEach(function (el) {
					if ($(input).prev().val().toLowerCase().indexOf(el.toLowerCase()) >= 0) {
						if (!user_is_parther && user.COUNTRY_NAME.toLowerCase() == el.toLowerCase()) {
							errors.push('В выбранной стране (' + el + ') заказ не доступен.');
						}
					}
				});
			}
		...
		editTotalBlock: function()
		{
			...
			var total = this.result.TOTAL,
				priceHtml = '<span class="bx-price">' + total.PRICE_WITHOUT_DISCOUNT + '</span>',
				discText, bonusText, valFormatted, i,
			...
			if (this.options.showBonus)
			{
				...
				bonusText = BX.message('SOA_SUM_BONUS');
				this.totalInfoBlockNode.appendChild(this.createTotalUnit(bonusText + ':', numbFmt + ' руб.'));
			...
			else if($('input[name="ORDER_PROP_45"]') && (!enroll && uscash)){
			...
			else if((this.options.showBonus || this.options.showPriceWithoutDiscount) && total.CASH){
				this.totalInfoBlockNode.appendChild(this.createTotalSummWithDiscontBonus(BX.message('SOA_SUM_IT'), total.ORDER_TOTAL_PRICE, total.CASH.enroll, total.PRICE_WITHOUT_DISCOUNT_VALUE));
			}
			else
		...
		createTotalSummWithDiscontBonus: function(name, totalPriceValue, bonus, diskont){
			bonus = bonus ? bonus : 0;
			diskont = diskont ? diskont : 0;
			const formatFinishSumm = new Intl.NumberFormat('ru-RU');
			return BX.create('DIV', {
				props: {className: 'bx-soa-cart-total-line bx-soa-cart-total-line-total'},
				children: [
					...
					BX.create('DIV', {
						...
						children: [
							BX.create('SPAN', {text: this.options.showBonus ? formatFinishSumm.format(Math.floor(totalPriceValue - Number(bonus))) + ' руб.' : formatFinishSumm.format(Math.floor(totalPriceValue)) + ' руб.'})
						]
					})
				]
			});
		...
		editButtonSaveBlock: function(){
			if(!this.result.SHOW_AUTH)
			{
				var showOrderButton = this.params.SHOW_TOTAL_ORDER_BUTTON == 'Y';
				this.finalBtnSaveBlock.appendChild(
					BX.create('DIV', {
						...
						children: [
							BX.create('A', {
								props: {
									...
									className: 'external btn btn-default btn-lg btn-order-save'
								},
