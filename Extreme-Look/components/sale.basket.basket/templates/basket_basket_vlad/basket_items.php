...
<script>
    var partner_min_summ = <?=json_encode($_SESSION['MY_PARAMS'][1])?>,
        partner = <?=json_encode($getPartner)?>,
        newpartner = <?=json_encode($getNewPartner)?>;
</script>
...
<div id="basket_items_list">
	<?if($normalCount > 0) {?>
				...
				<?foreach($arResult["GRID"]["HEADERS"] as $arHeader) {
					...
					<?} elseif($arHeader["id"] == "SUM") {?>
						<div class="basket-item-td">
                            ...
                            <span class="basket-item-discount" id="BONUS_PRICE_ALL_FORMATED" style="display: none"></span>
						</div>
					<?} else {?>
        ...
        <div data-partner="<?=$getPartner?>" data-summ="<?=$_SESSION["MY_PARAMS"][1]?>" style='display: none' id=by_ajax_id></div>
        <? //Мой код начало если сумма покупки меньше минимальной и пользователь партнёр выводим ошибку и скрываем кнопки покупки ?>
        <?if($arResult['allSum'] < $_SESSION["MY_PARAMS"][1] && $getPartner){?>
            ...
            <div class="hidden-print bx_ordercart_order_pay" style="display: none" id="id_my_ajax">
                ...
                <div class="bx_ordercart_order_pay_right">
                    ...
                    <?}
                    if($arParams["QUICK_ORDER"] && !$getPartner) {?>
        ...
        <?}elseif(!$getPartner || ($arResult['allSum'] > $_SESSION["MY_PARAMS"][1] && $getPartner)){?>
            ...
            <div class="hidden-print bx_ordercart_order_pay" style="display: block" id="id_my_ajax">
                <?if(getUserCashBack()){
                    $cash = getCash($USER->GetID(), $arResult["allSum"]);?>
                    <div class="bx_ordercart_order_cash_left" id="cash_block">
                        <div class="cash-name">Баллы и бонусы <i class="question-ex" data-tooltip="<?=GetMessage("SALE_BONUS_TOOLTIP")?>"></i></div>
                        <div class="cash-block">
                            <div class="write-off active" id="CASH_WRITE_OFF">Начислить: <?=$cash["write"]?></div>
                            <div class="enroll" id="ENROLL">Списать: <?=$cash["enroll"]?></div>
                        </div>
                    </div>
                    <input name="CASH" id="CASH_WR_EN" type="hidden" value="N">
                <?}?>
                <div class="bx_ordercart_order_pay_left" id="coupons_block">
                    <?//COUPON//
                    if($arParams["HIDE_COUPON"] != "Y") {?>
