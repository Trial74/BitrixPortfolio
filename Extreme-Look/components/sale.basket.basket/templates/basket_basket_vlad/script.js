...
function updateBasketTable(basketItemId, res) {
	...
	if(!!res.BASKET_DATA) {
		...
		if(BX('CASH_WRITE_OFF')){
			BX('CASH_WRITE_OFF').innerHTML = 'Начислить: ' + Math.floor((res['BASKET_DATA']['allSum'] - 169) * basketJSParams['CASH_BY_PARTNER']['cash']);
		}
		if(BX('ENROLL')){
			if(Number(basketJSParams['CASH_BY_PARTNER']['cash']) > (Number(res['BASKET_DATA']['allSum']) / 2)){
				BX('ENROLL').innerHTML = 'Списать: ' + Math.floor(res['BASKET_DATA']['allSum'] / 2);
			}
		}
...
function recalcBasketAjax(params) {
	...
	BX.showWait();
	const sumFormat = new Intl.NumberFormat('ru-RU');
	var property_values = {},
		action_var = BX('action_var').value,
	...
	BX.ajax({
		...
		onsuccess: function(result) {
			...
			if(!!params && (params == 'ENROLL' || params == 'CASH_WRITE_OFF')){

				if(Number(basketJSParams['CASH_BY_PARTNER']['enroll']) > (Number(result.BASKET_DATA.allSum) / 2))
					usBonus = Math.floor(result.BASKET_DATA.allSum / 2);
				else
					usBonus = basketJSParams['CASH_BY_PARTNER']['enroll'];

				if(params == 'ENROLL'){
					BX('CASH_WRITE_OFF').innerHTML = basketJSParams['SALE_BONUS_ENROLL'] + Math.floor((result.BASKET_DATA.allSum - usBonus - 169) * basketJSParams['CASH_BY_PARTNER']['cash']);
					BX.adjust(BX('BONUS_PRICE_ALL_FORMATED'), {
						text: basketJSParams['SALE_BONUS_MESS'] + ' ' + sumFormat.format(usBonus) + ' руб.'
					});
					BX.style(BX('BONUS_PRICE_ALL_FORMATED'), 'display', 'block');
					BX.adjust(BX('allSum_FORMATED'), {
						text: sumFormat.format(result.BASKET_DATA.allSum - usBonus) + ' руб.'
					})
				}
				if(params == 'CASH_WRITE_OFF'){
					BX('CASH_WRITE_OFF').innerHTML = basketJSParams['SALE_BONUS_ENROLL'] + Math.floor((result.BASKET_DATA.allSum - 169) * basketJSParams['CASH_BY_PARTNER']['cash']);
					BX.style(BX('BONUS_PRICE_ALL_FORMATED'), 'display', 'none');
					BX.adjust(BX('BONUS_PRICE_ALL_FORMATED'), {
						text: ''
					});
					BX.adjust(BX('allSum_FORMATED'), {
						text: result.BASKET_DATA.allSum_FORMATED
					})
				}
				...
				if(BX('ENROLL')){
					if(Number(basketJSParams['CASH_BY_PARTNER']['enroll']) > (Number(result.BASKET_DATA.allSum) / 2))
						BX('ENROLL').innerHTML = basketJSParams['SALE_BONUS_WRITE_OFF'] + Math.floor(result.BASKET_DATA.allSum / 2);
					else
						BX('ENROLL').innerHTML = basketJSParams['SALE_BONUS_WRITE_OFF'] + basketJSParams['CASH_BY_PARTNER']['enroll'];
				}
			}
		}
	});
...
function clickEnroll(data){
	BX.addClass(BX('ENROLL'), 'active');
	BX.removeClass(BX('CASH_WRITE_OFF'), 'active');
	BX.adjust(BX('CASH_WR_EN'), {props: {value: 'Y'}});

	recalcBasketAjax('ENROLL');
}

function clickCashWriteOff(data){
	BX.addClass(BX('CASH_WRITE_OFF'), 'active');
	BX.removeClass(BX('ENROLL'), 'active');
	BX.adjust(BX('CASH_WR_EN'), {props: {value: 'N'}});

	recalcBasketAjax('CASH_WRITE_OFF');
}
...
BX.ready(function() {
	...
	if(BX('CASH_WRITE_OFF') && BX('CASH_WR_EN')){
		BX.bind(BX('ENROLL'), 'click', BX.proxy(function () {
			clickEnroll(false);
		}, this));

		BX.bind(BX('CASH_WRITE_OFF'), 'click', BX.proxy(function () {
			clickCashWriteOff(false);
		}, this));
	}
