...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		init: function(parameters) {
			...
			this.cashPart = parameters.cashPart || '';
		...
		showError: function(node, msg, border) {
			...
			if(errorContainer && msg.length) {
				...
				if(animate) {
					...
					BX.addClass(errorContainer, 'alert-show');
		...
		checkNotifications: function() {
			...
			if(informer) {
				if(this.firstLoad && this.result.IS_AUTHORIZED && typeof this.result.LAST_ORDER_DATA.FAIL === 'undefined') {
					...
					BX.addClass(informer, 'alert alert-' + className + ' alert-show');
		...
		showWarnings: function() {
			...
			if((currentDelivery || currentDelivery.CALCULATE_ERRORS || parseFloat(total.ORDER_PRICE) === 0) && currentDelivery.ID != 3) {//Мой код сообщение о рассчёте доставки менеджером выводим на все доставки пока не настроится расчёт стоимости
		...
		showBlockWarning: function(node, warnings, hide) {
			...
			if(errorNode) {
				...
				warningNode = BX.create('DIV', {
					props: {className: 'alert alert-warning' + (!hide ? ' alert-show' : '')},
					html: warnStr
				});
		...
		initOptions: function() {
			...
			if(this.result.TOTAL) {
				...
				this.options.showBonus = cash === 'Y' ? true : false
			}
		},
		...
		markSectionAsCompleted: function(section) {
			...
			BX.remove(section.querySelector('.alert.alert-warning'));
		...
		fade: function(node, nextSection) {
			...
			switch(node.id) {
				...
				case this.regionBlockNode.id:
					...
					break;
				case this.paySystemBlockNode.id:
					BX.remove(this.paySystemBlockNode.querySelector('.alert.alert-warning'));
					this.editFadePaySystemBlock();
					break;
				case this.deliveryBlockNode.id:
					BX.remove(this.deliveryBlockNode.querySelector('.alert.alert-warning'));
		...
		editAuthBlock: function() {
			...
			if(this.result.OK_MESSAGE && this.result.OK_MESSAGE.length) {
				...
				okMessageNode = BX.create('DIV', {
					props: {className: 'alert alert-success alert-show'},
		...
		editActiveBasketBlock: function(activeNodeMode) {
			...
			if(this.initialized.basket) {
				...
				if(usercashback){
					this.addCashBlock(basketFooter);
				}
		...
		editBasketItemsHeader: function(basketItemsNode) {
			...
			for(i = 0; i < this.result.GRID.HEADERS.length; i++) {
				...
				if(column.id === 'NAME' || column.id === 'PREVIEW_PICTURE' || column.id === 'PROPS' || column.id === 'NOTES' || column.id == 'PROPERTY_ARTNUMBER_VALUE' || column.id == 'PROPERTY_OLD_PRICE_VALUE')
					continue;
		...
		createBasketItem: function(basketItemsNode, item, index, active) {
			...
			for(i = 0; i < this.result.GRID.HEADERS.length; i++) {
				...
				if(currentColumn.id === 'NAME' || currentColumn.id === 'PREVIEW_PICTURE' || currentColumn.id === 'PROPS' || currentColumn.id === 'NOTES' || currentColumn.id == 'PROPERTY_ARTNUMBER_VALUE' || currentColumn.id == 'PROPERTY_OLD_PRICE_VALUE')
					continue;
							...
							events: {
								change: BX.delegate(function(){
									...
									if(newCoupon && newCoupon.value) {
										if(newCoupon.value.length == 12) {//Мой код вводим карты лояльности если введено 12-значное число
											if(newCoupon.value.substr(0, 1) == 5){//Если первая цифра пять то применяем купон 5 процентов
												newCoupon.value = "Скидка по карте 5%";//Применяем купон 5 процентов
												this.sendRequest('enterCoupon', newCoupon.value);
											}
										}
										if(newCoupon.value.length == 13 && newCoupon.value !== 'EXTREME_BLACK' && newCoupon.value !== 'extreme_black') { //Мой код вводим карты лояльности если введено 13-значное число
											if(newCoupon.value.substr(1, 1) == 5){ //Если вторая цифра равна 5 то карта пять процентов
												newCoupon.value = "Скидка по карте 5%"; //Применяем купон пять процентов
												this.sendRequest('enterCoupon', newCoupon.value);
											}
											if(newCoupon.value.substr(0, 1) == 1){ //Если первая цифра равна 1 то карта десять процентов
												newCoupon.value = "Скидка по карте 10%"; //Применяем купон 10 процентов
												this.sendRequest('enterCoupon', newCoupon.value);
											}
										}
										else this.sendRequest('enterCoupon', newCoupon.value);
									}
								}, this)
							}
						}),
						BX.create('A', {
							...
							events: {
								click: BX.delegate(function(){
									var newCoupon = BX('coupon');
									if(newCoupon && newCoupon.value) {
										if(newCoupon.value.length == 12) { //Мой код дубль комменты выше
		...
		cashWriteOff: function(){
			var totalBlockBonus = document.querySelectorAll("#SOA_BONUS"),
				finalSum = document.querySelectorAll('.bx-soa-cart-total-line-total > .bx-soa-cart-d');
			BX.addClass(BX('CASH_WRITE_OFF'), 'active');
			BX.removeClass(BX('ENROLL'), 'active');
			BX.adjust(BX('CASH_WRITE_OFF'), {text: BX.message('SOA_BONUS_WRITE_OFF') + Math.round(((Math.round(this.result.TOTAL.ORDER_PRICE) - 169) * this.cashPart.cash))});
			BX.adjust(BX('SOA_BONUS'), {text: ''});
			BX.adjust(BX(totalBlockBonus[0]), {text: ''});
			BX.adjust(BX(totalBlockBonus[1]), {text: ''});
			BX.adjust(BX(finalSum[0]), {text: this.result.TOTAL.ORDER_PRICE_FORMATED});
			BX.adjust(BX(finalSum[1]), {text: this.result.TOTAL.ORDER_PRICE_FORMATED});
			$('input[name="ORDER_PROP_45"]').val('Списать ' + 0 + ' и начислить ' + this.cashPart.write);
		},
		cashEnroll: function(){
			var totalBlockBonus = document.querySelectorAll("#SOA_BONUS"),
				finalSum = document.querySelectorAll('.bx-soa-cart-total-line-total > .bx-soa-cart-d');
			...
			BX.adjust(BX('CASH_WRITE_OFF'), {text: BX.message('SOA_BONUS_WRITE_OFF') + Math.round(((Math.round(this.result.TOTAL.ORDER_PRICE) - this.cashPart.enroll - 169) * this.cashPart.cash))});
			BX.adjust(BX(totalBlockBonus[0]), {text: BX.message('SOA_SUM_BONUS') + ' ' + this.cashPart.enroll});
			BX.adjust(BX(totalBlockBonus[1]), {text: BX.message('SOA_SUM_BONUS') + ' ' + this.cashPart.enroll});
			BX.adjust(BX(finalSum[0]), {text: this.result.TOTAL.ORDER_PRICE - this.cashPart.enroll + ' руб.'});
			BX.adjust(BX(finalSum[1]), {text: this.result.TOTAL.ORDER_PRICE - this.cashPart.enroll + ' руб.'});
			$('input[name="ORDER_PROP_45"]').val('Списать ' + this.cashPart.enroll + ' и начислить ' + Math.floor((this.result.TOTAL.ORDER_PRICE - this.cashPart.enroll - 169) * this.cashPart.cash));
		},
		addCashBlock: function(basketItemsNode){
			basketItemsNode.appendChild(
				BX.create('DIV', {
					props: {
						className: 'bx_ordercart_order_cash',
						id: 'cash_block'},
					children: [
						BX.create('DIV', {
							props: {className: 'bx-soa-cash-name'},
							children: [
								BX.create('div', {
									text: BX.message('SOA_BONUS')
								}),
								BX.create('i', {
									props: {
										className: 'soa-question-ex'
									},
									dataset: {
										tooltip: BX.message('SOA_BONUS_TOOLTIP')
									}
								})
							]
						}),
						BX.create('DIV', {
							props: {className: 'bx-soa-cash-block'},
							children:[
								BX.create('DIV', {
									props: {
										className: 'bx-soa-write-off ' + (cash == 'N' || !cash ? 'active' : ''),
										id: 'CASH_WRITE_OFF'
									},
									text: BX.message('SOA_BONUS_WRITE_OFF') + this.cashPart.write,
									events: {
										click: BX.proxy(this.cashWriteOff, this)
									},
								}),
								BX.create('DIV', {
									props: {
										className: 'bx-soa-enroll ' + (cash == 'Y' ? 'active' : ''),
										id: 'ENROLL'
									},
									text: BX.message('SOA_BONUS_ENROLL') + this.cashPart.enroll,
									events: {
										click: BX.proxy(this.cashEnroll, this)
									},
								})
							]
						})
		...
		createPaySystemItem: function(item) {
			var checked = item.CHECKED == 'Y',
				...
				//pay = false,
				//newCouponGift = true,
				//lenghtCoup = this.result.COUPON_LIST.length,
				//close = false; //Мой код закрываем некоторые платёжки для сайта т.к. они для приложения со своими параметрами
			...
			/*
			if((paySystemId !== 14 && paySystemId !== 15 && paySystemId !== 16 && paySystemId !== 18 && paySystemId !== 26 && paySystemId !== 22 && paySystemId !== 24 && paySystemId !== 8 && paySystemId !== 25 && paySystemId !== 27 && paySystemId !== 28 && paySystemId !== 29 && paySystemId !== 30 && paySystemId !== 33) ||
			...
			*/
			...
			if(this.params.SHOW_PAY_SYSTEM_LIST_NAMES == 'Y') {
				title = BX.create('DIV', {
					props: {className: 'bx-soa-pp-company-smalltitle-container'},
					children: [
						BX.create('DIV', {
							props: {className: 'bx-soa-pp-company-smalltitle-block'},
							children: [
								BX.create('DIV', {
			...
			itemNode = BX.create('DIV', {
				...
				children: [
					BX.create('DIV', {
						props: {
							className: 'bx-soa-pp-company' + (checked ? ' bx-selected' : '')
						},
		...
		editPaySystemInfo: function(paySystemNode) {
			...
			var paySystemInfoContainer = BX.create('DIV', {props: {className: 'bx-soa-pp-company-info'}}),
			...
			if(this.result.PAY_FROM_ACCOUNT == 'Y') {
				...
				paySystemInfoContainer.appendChild(innerPs);
			}
			...
			if(currentPaySystem) {
				if(this.params.SHOW_PAY_SYSTEM_INFO_NAME == 'Y') {
					title = BX.create('DIV', {
						props: {className: 'bx-soa-pp-company-info-title'},
						text: currentPaySystem.NAME
					});
					paySystemInfoContainer.appendChild(title);
				}

				if(currentPaySystem.DESCRIPTION && currentPaySystem.DESCRIPTION.length) {
					desc = BX.create('DIV', {
						props: {className: 'bx-soa-pp-company-info-block'},
						children: [BX.create('DIV', {props: {className: 'bx-soa-pp-company-info-desc'}, html: currentPaySystem.DESCRIPTION})]
					});
					paySystemInfoContainer.appendChild(desc);
				}
				...
				if(currentPaySystem.PRICE && parseFloat(currentPaySystem.PRICE) > 0) {
					price = BX.create('DIV', {
						...
						children: [
							BX.create('DIV', {
								props: {
									className: 'bx-soa-pp-company-info-prop'
								},
								children: [
									BX.create('SPAN', {
										props: {className: 'bx-soa-pp-company-info-prop-title'},
										html: this.params.MESS_PRICE + ':'
									}),
									BX.create('SPAN', {
										props: {className: 'bx-soa-pp-company-info-prop-val'},
										html: '~' + currentPaySystem.PRICE_FORMATTED
									}),
								]
							})
						]
					});
					paySystemInfoContainer.appendChild(price);
				}
			}
		...
		getInnerPaySystem: function() {
					...
					BX.create('LABEL', {
						children: [
							...
							BX.create('SPAN', {
								...
								html: '<span class="bx-soa-pp-company-info-budget">' + this.result.CURRENT_BUDGET_FORMATED + '</span>' + paySystem.NAME
							})
						],
						events: {
							click: BX.proxy(this.selectPaySystem, this)
			...
			return BX.create('DIV', {
				props: {className: 'form-group bx-soa-pp-inner-ps' + (isSelected ? ' bx-selected' : '')},
		...
		editDeliveryInfo: function(deliveryNode) {
			...
			var deliveryInfoContainer = BX.create('DIV', {props: {className: 'bx-soa-pp-company-info'}}),
				currentDelivery, title, desc, calcDesc,
				extraServices, extraServicesNode,
				price, period, infoList;
			...
			if(this.params.SHOW_DELIVERY_INFO_NAME == 'Y') {
				title = BX.create('DIV', {
					props: {
						className: 'bx-soa-pp-company-info-title'
					},
					text: this.params.SHOW_DELIVERY_PARENT_NAMES != 'N' ? currentDelivery.NAME : currentDelivery.OWN_NAME
				});
				deliveryInfoContainer.appendChild(title);
			}

			if(currentDelivery.DESCRIPTION && currentDelivery.DESCRIPTION.length) {
				desc = BX.create('DIV', {
					props: {className: 'bx-soa-pp-company-info-block'},
					children: [
						BX.create('DIV', {props: {className: 'bx-soa-pp-company-info-desc'}, html: currentDelivery.DESCRIPTION})
					]
				});
				deliveryInfoContainer.appendChild(desc);
			}

			if(currentDelivery.CALCULATE_DESCRIPTION) {
				calcDesc = BX.create('DIV', {
					props: {className: 'bx-soa-pp-company-info-block'},
					children: [
						BX.create('DIV', {props: {className: 'bx-soa-pp-company-info-desc'}, html: currentDelivery.CALCULATE_DESCRIPTION})
					]
				});
				deliveryInfoContainer.appendChild(calcDesc);
			}
			...
			if(extraServices.length) {
				...
				deliveryInfoContainer.appendChild(extraServicesNode);
			}
			...
			/*if(currentDelivery.PRICE >= 0) {
				...
				deliveryInfoContainer.appendChild(price);
			}*/
			...
			if(currentDelivery.PERIOD_TEXT && currentDelivery.PERIOD_TEXT.length) {
				...
				deliveryInfoContainer.appendChild(period);
			}
		...
		createDeliveryItem: function(item) {
			...
			//Мой код начало, убираем временно цену доставки до подключения к обработчикам расчёта доставок
/*			price = BX.create('DIV', {props: {className: 'bx-soa-pp-delivery-cost-container'}});
					...
					if(inputs.length) {
						for(i = 0; i < inputs.length; i++) {
							if(inputs[i].value.length) {
								if(property.getSettings()['IS_PHONE'] == 'Y' && property.getSettings()['MULTIPLE'] != 'Y') {
		...
		alterProperty: function(settings, propContainer) {
			...
			if(textNode) {
				...
				if(settings.IS_PHONE == 'Y' && settings.MULTIPLE != 'Y') {
					if(settings.VALUE != '') {
		...
		showErrorTooltip: function(tooltipId, targetNode, text) {
			...
			if(tooltip.getAttribute('data-state') != 'opened') {
				...
				BX.addClass(tooltip, 'alert-show');
		...
		closeErrorTooltip: function(tooltipId) {
			...
			if(tooltip) {
				...
				new BX.easing({
					...
					complete: function() {
						BX.removeClass(tooltip, 'alert-show');
		...
		validateString: function(input, arProperty, fieldName) {
			...
			if(value.length) {
				...
				if(arProperty.IS_PHONE === 'Y'){
					...
					} //this.iti[arProperty.ID].getNumber() == '' && !this.iti[arProperty.ID].isValidNumber() Мой код. Убрал проверку валидности телефона - много жалоб пользователей
				}
			}
		...
		getLocationStringSity: function(node) {
			if(!node)
				return '';
			...
			locationSteps = node.querySelectorAll('.bx-ui-combobox-fake.bx-combobox-fake-as-input');
			if(locationSteps[2])
			locationString += locationSteps[2].innerHTML;
			return locationString;
		},
		...
		validateLocation: function(input, arProperty, fieldName) {
			...
			var parent = BX.findParent(input, {tagName: 'DIV', className: 'form-group'}),
				...
				valueSity = this.getLocationStringSity(parent),
				errors = [],
				field = !!fieldName ? BX.message('SOA_FIELD') + ' "' + BX.util.htmlspecialchars(arProperty.NAME) + '"' : BX.message('SOA_FIELD');
			if(!uspartner) {
				for (var i = specSity.length - 1; i >= 0; i--) { //Мой код проверка на город эксклюзив
					if (valueSity == specSity[i]) {
						errors.push(BX.message('MY_ERROR_SITY_FIRST') + valueSity + BX.message('MY_ERROR_SITY_SECOND'));
						return errors;
						break;
					}
				}
		...
		editTotalBlock: function() {
			...
			if (this.options.showBonus)
			{
				const numbFmt = new Intl.NumberFormat('ru-RU').format(total.CASH.enroll);
				priceHtml += '<span class="bx-price-discount" id="SOA_BONUS">' + BX.message('SOA_SUM_BONUS') + ' ' + numbFmt + ' руб.' + '</span>';

				if($('input[name="ORDER_PROP_45"]'))
				{
					$('input[name="ORDER_PROP_45"]').val('Списать ' + total.CASH.enroll + ' и начислить ' + Math.floor((total.ORDER_TOTAL_PRICE - total.CASH.enroll - 169) * total.CASH.cash));
				}
			}
			else if($('input[name="ORDER_PROP_45"]') && cash && typeof total.CASH !== 'undefined'){
				priceHtml += '<span class="bx-price-discount" id="SOA_BONUS"></span>';
				$('input[name="ORDER_PROP_45"]').val('Списать ' + 0 + ' и начислить ' + total.CASH.write);
			}
			...
			if(deliveryError) {
				...
				if(parseFloat(total.DELIVERY_PRICE) === 0) {
					if(curDelivery.ID == 3)
						deliveryValue = BX.message('PRICE_FREE_DEFAULT');
					else
						deliveryValue = BX.message('SOA_NOT_CALCULATED');//Мой код временно поменял выводимое сообщение нулевой стоимости доставки this.params.MESS_PRICE_FREE;
			...
			if(this.options.showPayedFromInnerBudget) {
				...
				if(this.options.showBonus){
					const sumFormat = new Intl.NumberFormat('ru-RU').format(Number(total.ORDER_TOTAL_PRICE) - Number(total.CASH.enroll));
					this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_IT'), sumFormat + ' руб.', {total: true}));
				}
				else
			...
			var credit, href, close = false;
			if(uspartner){
				credit = Math.round((this.result.TOTAL.PRICE_WITHOUT_DISCOUNT_VALUE - (this.result.TOTAL.PRICE_WITHOUT_DISCOUNT_VALUE * 0.15)) / 6);
				href = 'https://extreme-look.ru/about/credit/s_by_business/';
			}
			else{
				credit = Math.round(this.result.TOTAL.ORDER_TOTAL_PRICE / 6);
				href = 'https://extreme-look.ru/about/credit/s_by_roz/';
			}
			if((!uspartner && this.result.TOTAL.ORDER_TOTAL_PRICE >= 5000 && false) || (uspartner && this.result.TOTAL.ORDER_TOTAL_PRICE >= 50000 && close)) { //Мой код выводим информацияю о рассрочке в финальный блок
				this.totalInfoBlockNode.appendChild(
					BX.create('DIV', {
						props: {className: 'bx-soa-cart-total-line'},
						children: [
							BX.create('SPAN', {props: {className: 'bx-soa-cart-t'}, text: 'Рассрочка:'}),
							BX.create('SPAN', {
