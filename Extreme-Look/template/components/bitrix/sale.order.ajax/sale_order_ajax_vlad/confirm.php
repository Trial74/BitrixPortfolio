			...
			foreach($arResult["PAYMENT"] as $payment){
                if($payment["PAY_SYSTEM_ID"] == 14){
                    ...
                    <table>
                        ...
                        <tr>
                            <td>
                                <div class="container"><b>Для продолжения нажмите "Оформить рассрочку" и заполните форму для заявки.</b></div>
                            </td>
                        </tr>
                        <tr>
                            <td style="padding: 10px;">
                                <form target="_blank" id="start_tinkoff"
                                    ...
                                    <input type="submit" style="padding: 10px; color: #fff; border: none" class="btn btn-buy" value="Оформить рассрочку"/>
                                </form>
                            </td>
                        </tr>
                    </table>
                        ...
                        <tr>
                            <td>
                                <form id="pay" name="pay" method="POST" action="https://paymaster.ru/payment/init">
                                    <input type="hidden" name="LMI_PAYMENT_AMOUNT" value="<?= $arResult['ORDER']['PRICE'] ?>">
                                    <input type="hidden" name="LMI_CURRENCY" value="RUB">
                                    <input type="hidden" name="LMI_PAYMENT_DESC" value="Оплата платежа №-<?=$arResult['ORDER']['ID']?>">
                                    <input type="hidden" name="LMI_PAYMENT_NO" value="<?= $arResult['ORDER']['ID'] ?>">
                                    <input type="hidden" name="LMI_MERCHANT_ID" value="11a7dba7-b54b-4ce8-9ff3-b9930774896b">
                                    <input type="hidden" name="LMI_SUCCESS_URL" value="https://extreme-look.ru/personal/order/payment/paymaster_success.php">
                                    <input type="hidden" name="LMI_FAIL_URL" value="https://extreme-look.ru/personal/order/payment/paymaster_fail.php">
                                    ...
                                    <?foreach ($basket as $basketItem) {?>
                                        ...
                                        <input type="hidden" name="LMI_SHOPPINGCART.ITEM[<?= $i; ?>].NAME" value="<?=htmlspecialcharsbx($basketItem->GetField("NAME"));?>">
                                        <input type="hidden" name="LMI_SHOPPINGCART.ITEM[<?= $i; ?>].QTY" value="<?=$basketItem->GetField("QUANTITY");?>">
                                        <input type="hidden" name="LMI_SHOPPINGCART.ITEM[<?= $i; ?>].PRICE" value="<?=$basketItem->GetField("PRICE");?>">
                                        <input type="hidden" name="LMI_SHOPPINGCART.ITEM[<?= $i; ?>].TAX" value="no_vat">

                                        <?$i++;
                                    }
                                    if($fields["PRICE_DELIVERY"] > 0){?>
                                        <input type="hidden" name="LMI_SHOPPINGCART.ITEM[<?= $i; ?>].NAME" value="Доставка товара">
                                        <input type="hidden" name="LMI_SHOPPINGCART.ITEM[<?= $i; ?>].QTY" value="Количество">
                                        <input type="hidden" name="LMI_SHOPPINGCART.ITEM[<?= $i; ?>].PRICE" value="<?=$fields["PRICE_DELIVERY"];?>">
                                        <input type="hidden" name="LMI_SHOPPINGCART.ITEM[<?= $i; ?>].TAX" value="no_vat">
                                    <?}?>
                ...
                <?}elseif($payment["PAY_SYSTEM_ID"] == 22){
                    if ($payment["PAID"] != 'Y') {
                ...
                if (!empty($arResult["PAYMENT"])) {
                    foreach ($arResult["PAYMENT"] as $payment) {
                        ...
                        if ($payment["PAY_SYSTEM_ID"] == 22) break;
