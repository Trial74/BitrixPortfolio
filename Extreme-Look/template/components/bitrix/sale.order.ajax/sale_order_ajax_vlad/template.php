...
?>
    <script>
        var specSity = <?=json_encode($specSity)?>,
            ...
            usnewpartner = <?=json_encode(getNewPartner())?>,
            usercashback = <?=getUserCashBack() ? json_encode(true) : json_encode(false)?>,
            totalordersumm = <?=json_encode($_SESSION["MY_PARAMS"][1])?>,
            administrator = <?=json_encode($_SESSION["MY_PARAMS"][2])?>,
            cash = <?=getUserCashBack() ? json_encode($_SESSION["SALE_BASKET_CASH"]) : json_encode(false)?>;
    </script>
<?
if($arResult['JS_DATA']['TOTAL']['ORDER_PRICE'] < $_SESSION["MY_PARAMS"][1] && getPartner() && empty($_GET['ORDER_ID']))
...
$arParams['INITIAL_COUNTRY'] = strtolower($arSettings['FORMS_DEFAULT_COUNTRY']['DEFAULT']);
...
<?if(strlen($request->get('ORDER_ID')) > 0) {
...
} elseif($arParams['MIN_ORDER_SUM'] > 0 && $arResult['ORDER_PRICE'] < $arParams['MIN_ORDER_SUM']) {
		...
		<div id="bx-soa-order" class="row" style="opacity: 0;">
			...
			<div class="col-md-9 bx-soa">
				...
				<div id="bx-soa-properties" data-visited="false" class="bx-soa-section bx-active">
					<div class="bx-soa-section-title-container">
						<div class="bx-soa-section-title">
							<span class="bx-soa-section-title-icon"><i class="icon-user-gray"></i><i class="icon-ok-circle"></i><i class="icon-danger"></i><i class="icon-exclamation-circle"></i></span>
	...
	<script>
		...
		BX.Sale.OrderAjaxComponent.init({
            ...
            cashPart: <?=CUtil::PhpToJSObject(getCash($USER->GetID(), $arResult['JS_DATA']['TOTAL']['ORDER_PRICE']))?>,
