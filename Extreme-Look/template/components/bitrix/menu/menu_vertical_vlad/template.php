...
if(!$isSiteClosed && !empty($arResult)) {?>
    <style>
        ul.catalog-menu::-webkit-scrollbar{
            width: 4px;
        }
        ul.catalog-menu::-webkit-scrollbar-thumb{
            background: #b2c7cb;
        }
        ul.catalog-menu::-webkit-scrollbar-track{
            background: #e3ecef;
        }
    </style>
	<ul class="catalog-menu scrollbar-inner" id="<?=$containerName?>" data-entity="dropdown-menu">
		...
		foreach($arResult as $arItem) {
		    if(!$arItem["PARAMS"]["SECTION_HIDE"]) continue;
					...
					<a href="<?=$arItem['LINK']?>">
						...
						<?} elseif(is_array($arItem["PARAMS"]["PICTURE"])) {?>
							<span class="<?=($arItem['DEPTH_LEVEL'] > 2 ? 'hidden-md hidden-lg ' : '')?>catalog-menu-pic">
								...
								<img src="<?=$arItem['PARAMS']['PICTURE']['SRC']?>" width="<?=$arItem['PARAMS']['PICTURE']['WIDTH']?>" height="<?=$arItem['PARAMS']['PICTURE']['HEIGHT']?>" alt="<?=$arItem['PARAMS']['PICTURE']['ALT']?>" title="<?=$arItem['PARAMS']['PICTURE']['TITLE']?>" onload="loadImg(this)" />
							</span>
						<?}?>
