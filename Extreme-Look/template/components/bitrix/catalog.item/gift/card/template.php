		...
		<a target="_self" class="product-item-image" id="<?=$itemIds['PICT_ID']?>" href="<?=$item['DETAIL_PAGE_URL']?>" title="<?=$imgTitle?>"<?=($arParams["QUICK_VIEW"] == "FULL" ? " data-entity='quickView'" : "")?>>
            ...
            <div class="product-item-markers">
				<span class="product-item-marker-container">
					<span class="tooltip_v product-item-marker-24px">
                        <i class="present_tovar"></i>
                        <span class="tooltiptext_v" style="<?=(!empty($arMarker['BACKGROUND_1']) ? 'background:'.$arMarker['BACKGROUND_1'] : 'background: #7b66fe')?>"><?=$arParams["TEXT_LABEL_GIFT"]?></span>
                    </span>
				</span>
            </div>
			<?//BRAND//
			...
			<div class="product-item-info">
				...
				<div class="product-item-button-container" data-entity="buttons-block">			
					<?if(($object && !$objectContacts) || $partnersUrl || ($haveOffers && (($arParams['OFFERS_VIEW'] != 'PROPS' && $arParams['OFFERS_VIEW'] != 'DROPDOWN_LIST') || $arParams['PRODUCT_DISPLAY_MODE'] != 'Y')) || $arParams['DISABLE_BASKET']) {?>
						<a target="<?=$item['TARGET']?>" class="btn btn-buy" href="<?=$item['DETAIL_PAGE_URL']?>" title="<?=$arParams['MESS_BTN_DETAIL']?>"<?=($arParams["QUICK_VIEW"] == "FULL" ? " data-entity='quickView'" : "")?>><i class="icon-arrow-right"></i></a>
					<?} else {
						if(!$haveOffers) {
							if($actualItem['CAN_BUY'] || (!$actualItem['CAN_BUY'] && !$showSubscribe)) {?>
								<div id="<?=$itemIds['BASKET_ACTIONS_ID']?>">
									<button type="button" class="btn btn-buy" id="<?=$itemIds['BUY_LINK']?>" title="<?=($arParams['ADD_TO_BASKET_ACTION'] === 'BUY' ? $arParams['GIFTS_MESS_BTN_BUY'] : $arParams['GIFTS_MESS_BTN_ADD_TO_BASKET'])?>"<?=($actualItem['CAN_BUY'] ? '' : ' disabled="disabled"')?>><i class="icon-cart"></i></button>
								</div>
							...
							<div id="<?=$itemIds['BASKET_ACTIONS_ID']?>">
								<button type="button" class="btn btn-buy" id="<?=$itemIds['BUY_LINK']?>" title="<?=($arParams['ADD_TO_BASKET_ACTION'] == 'BUY' ? $arParams['GIFTS_MESS_BTN_BUY'] : $arParams['GIFTS_MESS_BTN_ADD_TO_BASKET'])?>"<?=(!$offerPartnersUrl ? ($actualItem['CAN_BUY'] ? '' : ($showSubscribe ? ' style="display: none;"' : ' disabled="disabled"')) : ' style="display: none;"')?>><i class="icon-cart"></i></button>
							</div>
