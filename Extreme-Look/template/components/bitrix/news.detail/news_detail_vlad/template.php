	...
	if(!empty($arResult["PREVIEW_TEXT"]) || is_array($arResult["DETAIL_PICTURE"]) || !empty($arResult["DETAIL_TEXT"])) {?>
		<div class="news-item-detail">
            <div class="pic-name-container">
                ...
                <?}
                //ITEM_NAME_TEXT//?>
                <div class="block-name-button">
                    <?if(!empty($arResult["NAME"])) {?>
                        <div class="news-name-text"><?=$arResult["NAME"]?></div>
                    <?}?>
                    <?if(!empty($arResult["PROPERTIES"]["LINK_BY_CAT"]["VALUE"])){?>
                        <a class="btn btn-buy news-link-by-cat" href="<?=$arResult["PROPERTIES"]["LINK_BY_CAT"]["VALUE"]?>" style="margin: 10px 5%;"><span>Посмотреть в каталоге</span></a>
                    <?}?>
                </div>
            </div>
            <hr>
